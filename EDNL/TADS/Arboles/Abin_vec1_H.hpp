...
template <typename T>
Abin<T>::Abin(size_t maxNodos, const T& e_nulo) : nodos(new T[maxNodos]), maxNodos(maxNodos), ELTO_NULO(e_nulo){
...
template <typename T>
Abin<T>::Abin(const Abin<T>& A) : nodos(new T[A.maxNodos]), maxNodos(A.maxNodos), ELTO_NULO(A.ELTO_NULO){
